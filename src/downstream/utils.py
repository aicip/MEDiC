...
class WandbLogger(object):
    def __init__(self, project_name="medic-downstream", config=None, name=None, entity=None, wandb_dir=None):
        # Initialize wandb if not already done
        if wandb.run is None:
            # Set wandb directory to control where logs are stored
            # This prevents wandb folder from being created in the working directory
            init_kwargs = dict(project=project_name, entity=entity, config=config, name=name)
    ...
    def update(self, head="scalar", step=None, **kwargs):
        ...
        for k, v in kwargs.items():
            ...
            if isinstance(v, torch.Tensor):
                v = v.item()
            assert isinstance(v, (float, int)), f"Expected float or int, got {type(v)} for key {k}"
            # Don't add head prefix if it's already a flat metric like train_loss, test_acc1
            if "/" in k or head == "scalar":
                log_dict[k] = v
            else:
                log_dict[head + "/" + k] = v
        
        wandb.log(log_dict, step=self.step if step is None else step)
    ...
    def flush(self):
        # WandB automatically handles flushing, no action needed
        pass
...
def save_on_master(*args, **kwargs):
    if is_main_process():
        # Add _use_new_zipfile_serialization=False for weights_only compatibility
...
def load_state_dict(
    ...
    unexpected_keys = []
    error_msgs = []
    loaded_keys = []  # New list to store successfully loaded keys
    ...
    # Identify successfully loaded keys
    ...
    if len(loaded_keys) > 0:
        print(
            "Successfully loaded {} weights for {}".format(
                len(loaded_keys), model.__class__.__name__
            )
        )
    else:
        raise ValueError("No weights loaded for {}".format(model.__class__.__name__))
...
def save_model(
    ...
    if not getattr(args, "enable_deepspeed", False):
        if epoch == "best":
            # For best model, save only model weights (no optimizer/scheduler)
            ...
            # Regular checkpoints - not saved for finetune/linear probe (only best model)
            pass
    else:
...
def derive_wandb_name_from_checkpoint(checkpoint_path, task_prefix):
    """
    Derive WandB name from checkpoint path with descriptive format.

    Args:
        checkpoint_path: Path to checkpoint file
        task_prefix: 'fin' for finetuning, 'lin' for linear eval, 'seg' for semantic seg

    Returns:
        WandB name string

    Example:
        checkpoint: output/pretrain/medic_full_26_04_04_10/checkpoint-epoch0299.pth
        -> semseg-eval-E299-medic_full_26_04_04_10
    """
    import os
    import re

    # Extract the directory name containing the pretrain run name
    ...
    if epoch_match:
        ...
        # Try iter format
        iter_match = re.search(r'iter_(\d+)', checkpoint_filename)
        if iter_match:
            epoch_num = int(iter_match.group(1))
        else:
            epoch_num = 0

    # Map task prefix to descriptive name
    task_map = {
        'fin': 'finetune-eval',
        'lin': 'linear-eval',
        'seg': 'semseg-eval',
    ...
    # Format: semseg-eval-E299-medic_full_26_04_04_10
    return f"{task_name}-E{epoch_num}-{run_name}"
